...
static DATE_REGEX: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(\d{1,2})\s+(Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?|Apr(?:il)?|May|Jun(?:e)?|Jul(?:y)?|Aug(?:ust)?|Sep(?:tember)?|Oct(?:ober)?|Nov(?:ember)?|Dec(?:ember)?)\s+(\d{4})").unwrap()
});
...
pub async fn rss_feed(_app_state: web::Data<AppState>) -> Result<HttpResponse, actix_web::Error> {
    ...
    for item in items {
        ...
        let pub_date = item.date.map(|d| d.format("%a, %d %b %Y").to_string());

        let description = item
            .description
            .unwrap_or_else(|| "Read more about this content".to_string());
...
fn collect_content_items(dir: &Path, items: &mut Vec<ContentItem>) -> Result<(), actix_web::Error> {
    if dir.is_dir() {
        for entry in fs::read_dir(dir)
            .map_err(|_| actix_web::error::ErrorInternalServerError("Failed to read directory"))?
        {
            let entry = entry.map_err(|_| {
                actix_web::error::ErrorInternalServerError("Failed to read directory entry")
            })?;
            let path = entry.path();
...
fn process_markdown_file(file_path: &Path) -> Result<Option<ContentItem>, actix_web::Error> {
    ...
    if let JsonValue::Object(map) = frontmatter {
        ...
        let date_str = map.get("date").and_then(|v| v.as_str());

        let date = date_str.and_then(|ds| parse_date(ds)).map(|nd| {
        ...
        let rel_path = file_path.strip_prefix("content").map_err(|_| {
            actix_web::error::ErrorInternalServerError("Failed to get relative path")
        })?;
